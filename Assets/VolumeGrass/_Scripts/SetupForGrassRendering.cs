...
public class SetupForGrassRendering : MonoBehaviour {
	...
	private void SetupParams() {
		camera.cullingMask=camera.cullingMask&(~(1<<GrassRenderingReservedLayer.layer_num));
		myCam.CopyFrom(camera);
		myCam.targetTexture=myRenderTexture;
		myCam.cullingMask=cullingMask|(1<<GrassRenderingReservedLayer.layer_num);
		myCam.depth=camera.depth-1;
		...
		//if (Application.platform==RuntimePlatform.WindowsEditor || Application.platform==RuntimePlatform.WindowsPlayer || Application.platform==RuntimePlatform.WindowsWebPlayer) {
			...
			zc0 = 1.0f - FarClip / camera.nearClipPlane;
			zc1 = FarClip / camera.nearClipPlane;
		//} else {
			...
			if (deltaSqrM<teleportTresholdSqr) {
				...
				for(int i=0; i<motionBlurGrassObjects.Length; i++) {
					...
					if (obj.renderer && obj.renderer.material) {
						obj.renderer.material.SetVector("_mblur_dir", deltaM);
						obj.renderer.material.SetFloat("_mblur_val", len);
					}
				}
