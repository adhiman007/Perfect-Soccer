...
public class VolumeGrass : MonoBehaviour {
	...
	public void checkOGL() {
		...
		if (Application.isPlaying) {
			if (renderer && renderer.material) {
				shader_maxLOD=renderer.material.shader.maximumLOD;
				if ((!_compatibility()) || useOGL) {
					...
					renderer.material.shader=Shader.Find("Grass Shader ALT"); // don't use optimized subshader
					// disable mipmaping (if present)
					//disableMIPs(renderer.material, "_BladesTex");
					//disableMIPs(renderer.material, "_BladesBackTex");
				} else {
					renderer.material.shader=Shader.Find("Grass Shader"); // use optimized version
				}
				renderer.material.shader.maximumLOD=shader_maxLOD;
			}
		} else {
			if (renderer && renderer.sharedMaterial) {
				shader_maxLOD=renderer.sharedMaterial.shader.maximumLOD;
				if ((!_compatibility()) || useOGL) {
					renderer.sharedMaterial.shader=Shader.Find("Grass Shader ALT");
				} else {
					renderer.sharedMaterial.shader=Shader.Find("Grass Shader"); // use optimized version
				}
				renderer.sharedMaterial.shader.maximumLOD=shader_maxLOD;
			}
		}
	}
	/*
	...
	*/
	void Start() {
		...
		if (renderer && renderer.material) {
			renderer.material.SetFloat("_ZBufferParamA", SetupForGrassRendering.ZBufferParamA);
			renderer.material.SetFloat("_ZBufferParamB", SetupForGrassRendering.ZBufferParamB);
			renderer.material.SetFloat("_ZBufferFarClip", SetupForGrassRendering.ZBufferFarClip-1);
			renderer.material.SetVector("_auxtex_tiling", new Vector4(1.0f/(maxx-minx)/UV_ratio, 1.0f/(maxz-minz)/UV_ratio, -minx*UV_ratio,-minz*UV_ratio));
			renderer.material.SetFloat("_tiling_factor", TilingFactors[act_lod]);
		}
		if (fullBackGeometry) {
			...
			if (mf && my_mf) {
			    ...
			    mf.mesh = msh;
				go.renderer.material=new Material(Shader.Find("GrassRenderDepth"));
	...
	public void setupLODAndShader(int lod, bool shared_flag) {
		...
		if (shared_flag) {
			...
			if ((renderer!=null) && (renderer.sharedMaterial!=null)) {
				renderer.sharedMaterial.SetFloat("_tiling_factor", TilingFactors[lod]);
				renderer.sharedMaterial.SetFloat("PLANE_NUM", plane_num);
				renderer.sharedMaterial.SetFloat("GRASS_SLICE_NUM", 1.0f*slices_num);
				renderer.sharedMaterial.SetFloat("PLANE_NUM_INV", (1.0f/plane_num));
				renderer.sharedMaterial.SetFloat("GRASS_SLICE_NUM_INV", (1.0f/slices_num));
				renderer.sharedMaterial.SetFloat("GRASSDEPTH", (1.0f/slices_num));
				renderer.sharedMaterial.SetFloat("PREMULT", (plane_num/slices_num));
				renderer.sharedMaterial.SetFloat("UV_RATIO", UV_ratio);
				renderer.sharedMaterial.SetVector("_auxtex_tiling", new Vector4(1.0f/(maxx-minx)/UV_ratio, 1.0f/(maxz-minz)/UV_ratio, -minx*UV_ratio,-minz*UV_ratio));
			}
		} else {
			if (filter) filter.mesh=LODs[lod];
			if (filter_sidewalls) filter_sidewalls.mesh=LODs_sidewalls[lod];
			if ((renderer!=null) && (renderer.material!=null)) {
				renderer.material.SetFloat("_tiling_factor", TilingFactors[lod]);
			}				
		}
	...
	public bool BuildMesh() {
		...
		if (renderer.sharedMaterial!=null) {
			renderer.sharedMaterial.SetFloat("_tiling_factor", TilingFactors[act_lod]);
		} else {
			...
			if (_compatibility() && (!useOGL)) {
				renderer.sharedMaterial.shader=Shader.Find("Grass Shader"); // use d3d9/glsl version
			}			
			if (renderer.sharedMaterial) renderer.sharedMaterial.SetFloat("_tiling_factor", TilingFactors[act_lod]);
		}
		...
		if (sidewalls==null) {
			...
			go.renderer.sharedMaterial=new Material(Shader.Find("GrassRenderDepth"));
			go.renderer.receiveShadows=false;
			go.renderer.castShadows=false;
		} else {
	        ...
	        mf.sharedMesh = msh_sidewalls;
			if (go.renderer.sharedMaterial!=Shader.Find("GrassRenderDepth")) {
				go.renderer.sharedMaterial=new Material(Shader.Find("GrassRenderDepth"));
			}
			go.renderer.receiveShadows=false;
			go.renderer.castShadows=false;
	...
	public void showChecker() {
		...
		renderer.sharedMaterial=checkerMat;
	}
	public void hideChecker() {
		renderer.sharedMaterial=myMat;
	}
